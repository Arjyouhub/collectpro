...
Respond strictly in valid JSON format:
{
  ...
  "script": "Empathetic yet firm negotiation script for executive"
    ...
    let score = 85;
    if (caseData.dpd > 90) score -= 40;
    else if (caseData.dpd > 60) score -= 25;
    else if (caseData.dpd > 30) score -= 10;

    if (caseData.status === 'PTP') score += 15;
    if (caseData.status === 'Unreachable') score -= 20;
    if (caseData.status === 'Dispute') score -= 25;
   ...
   */
  static async generateScript(objectionType: string, customerName: string, amount: number): Promise<string> {
