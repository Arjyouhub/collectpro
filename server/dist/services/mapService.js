...
class MapService {
     ...
     */
    static optimizeRoute(origin, waypoints) {
        ...
        const waypointsStr = sorted
            ...
            .map(w => `${w.lat},${w.lng}`)
            .join('|');
        const googleNavUrl = `https://www.google.com/maps/dir/?api=1&origin=${origin.lat},${origin.lng}&destination=${destination}${waypointsStr ? `&waypoints=${waypointsStr}` : ''}&travelmode=driving`;
        return {
     ...
     */
    static getSingleNavigationUrl(lat, lng, address) {
        if (address) {
