...
class ExcelService {
     ...
     */
    static processRows(buffer, mapping, defaultPortfolioName, userId) {
        const workbook = XLSX.read(buffer, { type: 'buffer' });
        const firstSheetName = workbook.SheetNames[0];
        const worksheet = workbook.Sheets[firstSheetName];
        ...
        rawRows.forEach((row, idx) => {
            ...
            if (rawDPD > 90)
                ...
                bucket = '31-60 DPD';
            let priority = 'Medium';
            if (rawDPD > 90 || rawPOS > 100000)
                priority = 'Critical';
            else if (rawDPD > 60 || rawPOS > 50000)
                priority = 'High';
            processedCases.push({
                user: userId,
                accountNo,
